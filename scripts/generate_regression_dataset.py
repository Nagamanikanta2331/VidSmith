...
def generate_video(
    filename: str, vcodec: str, include_meta: bool = True, include_thumb: bool = True
):
    ...
    cmd.extend(["-c:v:0", vcodec, "-b:v", "500k", "-c:a", "aac", "-b:a", "128k", "-ar", "44100"])
...
def generate_audio(
    filename: str, ext: str, acodec: str, include_meta: bool = True, include_thumb: bool = True
):
    path = OUT_DIR / filename
    if path.exists():
        path.unlink()

    cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "sine=f=440:b=4:d=1"]
    if include_thumb:
        cmd.extend(["-i", str(ARTWORK)])
    ...
    cmd.extend(["-c:a", acodec, "-b:a", "128k", "-ar", "44100"])
    ...
    tasks = [
        ...
        (
            ...
            lambda: generate_video(
                "no_metadata.mp4", "libx264", include_meta=False, include_thumb=True
            ),
        ),
        (
            "no_thumbnail.mp4",
            lambda: generate_video(
                "no_thumbnail.mp4", "libx264", include_meta=True, include_thumb=False
            ),
        ),
        (
            "no_chapters.mp4",
            lambda: generate_video("no_chapters.mp4", "libx264"),
        ),  # Chapters not yet synthesized, but we create the file
        (
            "mp3_no_artwork.mp3",
            lambda: generate_audio(
                "mp3_no_artwork.mp3", ".mp3", "libmp3lame", include_meta=True, include_thumb=False
            ),
        ),
    ]
