...
from vidsmith.utils.console import err_console

_APP_DIR_NAME = "VidSmith"
# Config dir used before the project was renamed from MediaForge. Existing
# settings are copied (not moved) into the new location on first load so an
# upgrade never loses them; the old folder is left untouched as a fallback.
_LEGACY_APP_DIR_NAME = "MediaForge"
...
def settings_dir() -> Path:
    """Platform config directory for VidSmith."""
    return _config_dir(_APP_DIR_NAME)


def _config_dir(app_dir_name: str) -> Path:
...
def _legacy_settings_path() -> Path:
    return _config_dir(_LEGACY_APP_DIR_NAME) / _FILE_NAME


def _migrate_legacy_settings(path: Path) -> None:
    """One-time copy of a pre-rename MediaForge settings file.

    Runs only when the VidSmith settings file does not exist yet. The legacy
    file is copied, not moved, so the old install keeps working if both are
    ever present. Best-effort: any failure just means defaults are used.
    """
    if path.exists():
        return
