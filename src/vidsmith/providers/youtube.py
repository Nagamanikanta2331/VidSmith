...
from vidsmith.utils.environment import environment_summary, js_runtimes_option
from vidsmith.utils.exceptions import AnalysisError, DownloadError, UnsupportedURLError
...
_logger = logging.getLogger("vidsmith.provider.youtube")
...
class _SubtitleLogger:
    ...
    Everything yt-dlp says is also forwarded to the ``vidsmith`` debug log,
    so with Debug Logging enabled the full diagnostic stream (extraction steps,
    retries, JS-runtime output) lands in vidsmith.log instead of vanishing.
    """
    ...
    def __init__(self) -> None:
        ...
        self.last_error: str = ""
        self.logger = logging.getLogger("vidsmith.subtitle")
        self._ytdlp_logger = logging.getLogger("vidsmith.ytdlp")
...
class YouTubeProvider(Provider):
    ...
    def _embed_recovered_subtitles(
        ...
        """
        from vidsmith.subtitle import ISO_639_2
    ...
    def _log_stage_transition(self, progress: DownloadProgress) -> None:
        ...
        Produces a per-download timeline in vidsmith.log (extract 0.8s,
