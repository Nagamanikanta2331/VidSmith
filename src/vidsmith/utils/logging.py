"""Logging configuration for VidSmith."""
...
def configure_logging(debug: bool) -> None:
    """Configure the root logger for VidSmith.

    If debug is True, a RotatingFileHandler is attached writing to
    %APPDATA%/VidSmith/vidsmith.log.
    If debug is False, a NullHandler is attached.
    """
    root_logger = logging.getLogger("vidsmith")
    ...
    if debug:
        ...
        if appdata:
            log_dir = Path(appdata) / "VidSmith"
        else:
            xdg = os.environ.get("XDG_CONFIG_HOME")
            base = Path(xdg) if xdg else Path.home() / ".config"
            log_dir = base / "vidsmith"

        log_dir.mkdir(parents=True, exist_ok=True)
        log_file = log_dir / "vidsmith.log"
