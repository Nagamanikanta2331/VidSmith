...
from vidsmith.cli.summary.model import SummaryArtifactType, SummaryModel
from vidsmith.cli.summary.renderer import _elapsed_label, _format_bytes, _media_duration
...
from vidsmith.downloader.validators.models import DownloadValidationResult
from vidsmith.models.media import AnalysisResult
from vidsmith.providers.results import DownloadResult
from vidsmith.subtitle import SUBTITLE_LANGUAGE_NAMES, language_matches
...
def build_summary(
    ...
    if artifact_type == SummaryArtifactType.VIDEO:
        ...
        model.rows = [
            ...
            ),  # Just a placeholder since VidSmith doesn't pass a explicit flag for timestamps right now. Wait, Transcript job always strips timestamps unless requested, actually the python scraper does.
            ("File Size", _format_bytes(file_size) if file_size else ""),
        ]
