...
def test_execute_transcript_success(mock_show_error, mock_show_success, mock_engine, mock_validate, mock_get_provider, state, analysis_result, tmp_path):
...
@mock.patch("mediaforge.cli.executor.validate_download")
@mock.patch("mediaforge.cli.executor._show_error")
def test_execute_transcript_http_429(mock_show_error, mock_validate, mock_get_provider, state, analysis_result, tmp_path):
    state["output_dir"] = str(tmp_path)
    ...
    mock_validate.return_value = DownloadValidationResult(
        subtitle=SubtitleValidationResult(
            failed_languages={"en": "HTTP 429 (Rate Limited)"},
            success=False
        ),
        success=False
    )
...
@mock.patch("mediaforge.cli.executor.validate_download")
@mock.patch("mediaforge.cli.executor._show_error")
def test_execute_transcript_unavailable(mock_show_error, mock_validate, mock_get_provider, state, analysis_result, tmp_path):
    state["output_dir"] = str(tmp_path)
    ...
    mock_show_error.assert_called_once()
    assert mock_show_error.call_args[0][0] == "Transcript Download Failed"
    assert mock_show_error.call_args[0][1] == "Unavailable"

@mock.patch("mediaforge.cli.executor._get_provider")
@mock.patch("mediaforge.cli.executor.validate_download")
@mock.patch("mediaforge.transcript.engine.TranscriptEngine")
@mock.patch("mediaforge.cli.executor._show_error")
def test_execute_transcript_failed_conversion(mock_show_error, mock_engine, mock_validate, mock_get_provider, state, analysis_result, tmp_path):
    state["output_dir"] = str(tmp_path)
    ...
    mock_provider.download_transcript.return_value = DownloadResult(
        job_id="job123", url=analysis_result.url, status=DownloadResultStatus.COMPLETED,
        output_dir=tmp_path, media_type="transcript", files=[vtt_file]
    )
